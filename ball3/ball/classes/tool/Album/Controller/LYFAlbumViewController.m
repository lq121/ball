...
- (void)setupViewController {
    ...
    [LYFPhotoManger standardPhotoManger].choiceCountChange = ^(NSInteger choiceCount) {
        ...
        if (choiceCount == 0) {
            ...
            [weakSelf.confirmButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
        }
    };
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.row = self.albumModel.assets.count-1-indexPath.row;
    cell.asset = self.albumModel.assets[self.albumModel.assets.count-1-indexPath.row];
    [cell loadImage:[NSIndexPath indexPathForRow:self.albumModel.assets.count-1-indexPath.row inSection:indexPath.section]];
    cell.isSelect = [self.albumModel.selectRows containsObject:@(self.albumModel.assets.count-1-indexPath.row)];

    __weak typeof(self) weakSelf = self;
    __weak typeof(cell) weakCell = cell;
    cell.selectPhotoAction = ^(PHAsset *asset) {
        BOOL isReloadCollectionView = NO;
        if ([weakSelf.albumModel.selectRows containsObject:@(self.albumModel.assets.count-1-indexPath.row)]) {
            [weakSelf.albumModel.selectRows removeObject:@(self.albumModel.assets.count-1-indexPath.row)];
            ...
            [weakSelf.albumModel.selectRows addObject:@(self.albumModel.assets.count-1-indexPath.row)];
            [LYFPhotoManger standardPhotoManger].choiceCount++;
        ...
        if (isReloadCollectionView) {
            ...
            weakCell.isSelect = [weakSelf.albumModel.selectRows containsObject:@(self.albumModel.assets.count-1-indexPath.row)];
        }
    };
            ...
            for (NSNumber *row in albumModel.selectRows) {
                if (row.integerValue < albumModel.assets.count) {
                    ...
                    photoModel.getPictureAction = ^{
                        ...
                        if (photoList.count + [LYFPhotoManger standardPhotoManger].lastChoiceCount  == [LYFPhotoManger standardPhotoManger].choiceCount) {
                            button.enabled = YES;
