...
// 动画时间
const NSTimeInterval StripesAnimationTime = 0.03f;
const NSTimeInterval ProgressAnimationTime = 0.5f;

@interface RBAniProgress()
...
- (void)configDefaultProperty {
    ...
    _stripesOrientation = ProgressOrientationRight;
    _stripesWidth = 10.0f;
    _stripesColor = [UIColor colorWithSexadeString:@"#FFFFFF" AndAlpha:0.2];
    _stripesDelta = 5.0f;
    _hideStripes = NO;
    _stripesOffset = 0;
    // 轨迹
    _hideTrack = NO;
    _trackTintColor = [UIColor colorWithSexadeString:@"#D4D4D4"];
...
- (void)setProgress:(CGFloat)progress animated:(BOOL)animated {
    @synchronized (self) {
        ...
        if (animated) {
            ...
            CGFloat increment = ((_targetProgress - _progress) * StripesAnimationTime) / ProgressAnimationTime;
...
- (void)startStripeAnimationTimer {
    if (!_stripesTimer) {
        _stripesTimer = [NSTimer timerWithTimeInterval:StripesAnimationTime
                                                target:self
...
- (void)startProgressTimer:(NSNumber *)increment {
    if (!_progressTimer) {
        _progressTimer = [NSTimer timerWithTimeInterval:StripesAnimationTime
                                                 target:self
...
- (void)drawRect:(CGRect)rect {
    ...
    if (!_hideTrack) {
        // 背景轨迹
        [self drawTrack:context rect:CGRectMake(progressX, progressY, rectW - progressX * 2, 4)];
    }

    CGRect progressRect = CGRectMake(progressX, progressY, (rectW - progressX * 2) * _progress, 4);
    // 进度条
...
- (void)drawPoint:(CGContextRef)context x:(CGFloat)x y:(CGFloat)y h:(CGFloat)h space:(CGFloat)space {
...
- (void)drawCycle:(CGContextRef)context x:(CGFloat)x y:(CGFloat)y h:(CGFloat)h space:(CGFloat)space {
    CGContextSaveGState(context);
    {
        UIBezierPath *nodes = [UIBezierPath bezierPath];
...
- (void)setStripesOrientation:(ProgressOrientation)stripesOrientation {
