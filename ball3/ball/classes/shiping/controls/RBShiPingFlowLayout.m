...
@implementation RBShiPingFlowLayout
- (void)prepareLayout {
    [super prepareLayout];
    // 获取总的个数
    ...
    self.dataArr = [NSMutableArray arrayWithCapacity:kColCount];
    self.delegate = (id<UICollectionViewDelegateFlowLayout>)self.collectionView.delegate;

    //_dataArr 用来存放相邻的三个高度
    CGFloat top = .0f;
    for (NSUInteger idx = 0; idx < kColCount; idx++) {
        [_dataArr addObject:[NSNumber numberWithFloat:top]];
    }
    // 遍历所有的item，重新布局
...
- (void)layoutItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 遍历相邻三个高度获取最小高度
    NSUInteger col = 0;
    CGFloat shortHeight = [[_dataArr firstObject] floatValue];
    for (NSUInteger idx = 0; idx < _dataArr.count; idx++) {
        CGFloat height = [_dataArr[idx] floatValue];
    ...
    // 得到最小高度的当前Y坐标起始高度
    float top = [[_dataArr objectAtIndex:col] floatValue];
    // 设置当前cell的frame
    ...
    // 跟新dataArr数组中的高度
    [_dataArr replaceObjectAtIndex:col withObject:[NSNumber numberWithFloat:top + edgeInsets.top + itemSize.height]];
}
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    UICollectionViewLayoutAttributes * layoutFoot = [UICollectionViewLayoutAttributes   layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathWithIndex:attributes.count]];
    layoutFoot.frame =CGRectMake(0,0, RBScreenWidth, 65);
    [attributes addObject:layoutFoot];
    return attributes;
}
...
- (CGSize)collectionViewContentSize {
    ...
    CGFloat maxHeight = [[_dataArr firstObject] floatValue];

    //查找最高的列的高度
    for (NSUInteger idx = 0; idx < _dataArr.count; idx++) {
        CGFloat colHeight = [_dataArr[idx] floatValue];
